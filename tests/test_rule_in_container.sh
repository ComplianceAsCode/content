...
{
	local first_option all_short_options='nsdh'
	first_option="${1:0:1}"
...
print_help()
{
	...
	printf 'Usage: %s [-n|--name <arg>] [-s|--scenarios <arg>] [-d|--datastream <arg>] [-h|--help] <rule>\n' "$0"
	...
	printf '\t%s\n' "-d, --datastream: Path to the datastream to test. Autodetected by default. (no default)"
	printf '\t%s\n' "-h, --help: Prints help"
}
...
python "${script_dir}/test_suite.py" rule --remove-machine-only "${datastream_args[@]}" "${scenario_args[@]}" --add-platform "$test_image_cpe_product" --container "$_arg_name" -- "${_arg_rule}"
