...
def test_simple_match():
    ...
    assert rule._matches_platform(scenario_platforms, benchmark_cpes) == True


def test_simple_no_match():
    scenario_platforms = ["Red Hat Enterprise Linux 7"]
    benchmark_cpes = {"cpe:/o:redhat:enterprise_linux:6"}
    assert rule._matches_platform(scenario_platforms, benchmark_cpes) == False
...
def test_typo():
    ...
    with pytest.raises(ValueError):
        rule._matches_platform(scenario_platforms, benchmark_cpes)
