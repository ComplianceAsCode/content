...
def test_controls_load_product():
    # Assumes that the content root is at the same level as tests dir
    ssg_root = \
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))))
    env_yaml = {"product": "rhel8", "ssg_root": ssg_root}
    controls_manager = ssg.controls.ControlsManager(controls_dir, env_yaml)
    controls_manager.load()

    c_r1 = controls_manager.get_control("abcd", "R5")
    assert c_r1.title == "Enable FIPS mode"
    assert c_r1.description == "FIPS mode should be enabled"
    assert c_r1.automated == "yes"
    assert "grub2_enable_fips_mode" not in c_r1.rules
    assert "enable_fips_mode" in c_r1.rules
