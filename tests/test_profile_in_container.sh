...
# ARG_POSITIONAL_SINGLE([profile],[The short profile ID.])
# ARG_TYPE_GROUP_SET([remediations],[REMEDIATION],[remediate-using],[oscap,bash,ansible])
# ARG_DEFAULTS_POS([])
# DEFINE_SCRIPT_DIR([])
# ARG_HELP([Test a profile using the container backend.])
# ARGBASH_GO()
...
print_help()
{
	printf '%s\n' "Test a profile using the container backend."
	printf 'Usage: %s [-n|--name <arg>] [-d|--datastream <arg>] [-r|--remediate-using <REMEDIATION>] [--dry-run] [-h|--help] <profile>\n' "$0"
	printf '\t%s\n' "<profile>: The short profile ID. Wildcards are supported."
...
{
	local _required_args_string="'profile'"
...
assign_positional_args()
{
	...
	_positional_names="_arg_profile "
...
command=(python3 "${script_dir}/test_suite.py" profile --remove-machine-only "${additional_args[@]}" --add-platform "$test_image_cpe_product" --container "$_arg_name" -- "xccdf_org.ssgproject.content_profile_${_arg_profile}")
