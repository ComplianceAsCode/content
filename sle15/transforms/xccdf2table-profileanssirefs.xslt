...
<!-- this style sheet expects parameter , which is the id of the Profile to be shown -->
...
<xsl:include href="table-style.xslt"/>
	...
	<xsl:template match="/">
		<html>
		<head>
			<title><xsl:value-of select="/cdf:Benchmark/cdf:Profile[@id=]/cdf:title" /></title>
		</head>
		<body>
			<br/>
			<br/>
			<div style="text-align: center; font-size: x-large; font-weight:bold"><xsl:value-of select="/cdf:Benchmark/cdf:Profile[@id=]/cdf:title" /></div>
			<br/>
			<br/>
	...
	<xsl:template match="cdf:Benchmark">
		...
		<xsl:for-each select="/cdf:Benchmark/cdf:Profile[@id=]/cdf:select">
			...
			<xsl:for-each select="//cdf:Rule">
				<xsl:call-template name="ruleplate">
					<xsl:with-param name="idreference" select="" />
					<xsl:with-param name="enabletest" select="" />
				</xsl:call-template>
			</xsl:for-each>
		</xsl:for-each>
	...
	<xsl:template match="cdf:Rule" name="ruleplate">
		...
		<tr>
			...
			<td> 
				<xsl:for-each select="cdf:reference[@href=]">
	...
	<xsl:template match="cdf:check">
		<xsl:for-each select="cdf:check-export">
			<xsl:variable name="rulevar" select="@value-id" />
				<!--<xsl:value-of select="" />:-->
				<xsl:for-each select="/cdf:Benchmark/cdf:Profile[@id=]/cdf:refine-value">
					<xsl:if test="@idref=">
