...
Proceed with your work, if your work can be logically divided into multiple parts, try to structure your work so you avoid huge commits that affect logically unrelated parts of the project.
When you push to your forked repository for the first time, you will have to provide additional arguments to `git push`, because you are pushing a new branch to the remote repository: `git push -u origin my_new_feature`.
...
If you work on a feature or a bugfix that has an associated issue, either

- assign yourself to the issue (if you have rights), or
...
5) Keep historical records clean - don't use git history-altering magic in your branch during your work on the pull request.
...
You can always squash commits while merging the pull request, or you can close a complicated pull request, create a new one (in another branch) with streamlined commits and reference the old PR in the streamlined one, so it is possible to backtrack what went on.

6) A community member will review your patch. They will either merge the patch upstream, indicate additional changes/documentation needed, or decline the patch. You'll automatically be notified via EMail of any status updates.
