...
The products specify which benchmark(s) they use as a source of content in their `product.yml` file using `benchmark_root` key. It is a list and it may contain multiple paths. For example, `rhel7` product specifies that it uses the Linux OS benchmark.
