...
within the template directory. The file namess should be the same of the name of
the language in lower case, e.g. `oval`, `bash` etc.
...
an OVAL template file called _oval_ within the _package_installed_ directory:
...
And here is the Ansible template file called _ansible_ within the
...
2) Implement a preprocessing function which will process the
