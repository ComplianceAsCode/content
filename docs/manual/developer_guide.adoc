...
----
$ ./build_product rhel8
$ ./build-scripts/profile_stats.py stats --missing-fixes --profile ospp --benchmark build/ssg-rhel8-xccdf.xml
----

The tool also can create diff between YAML profiles and generate a new one.

For example, to generate a profile diff between two profiles, run this command:
----
$ ./build-scripts/profile_stats.py diff --profile1 rhel7/profiles/ospp.profile --profile2 rhel7/profiles/pci-dss.profile 
----

A new profile will be generated in the current directory with the name, in this case, ospp-compared_to-pci-dss.profile
