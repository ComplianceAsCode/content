...
# look for any occurrences of these attributes, and then gather the node referenced
...
def output_checks(dname):
    ...
    for defn, reflist in def_reflist_map.iteritems():
        ...
        fname = defn.get("id")
        fname = fname.translate(string.maketrans("",""), string.punctuation) + ".xml"
        # output definition, and then all elements that the definition references        
