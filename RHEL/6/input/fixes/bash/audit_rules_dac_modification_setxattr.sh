...
do
  ...
  EXPECTED_RULE="-a always,exit -F arch=${ARCH} -S setxattr -S lsetxattr -S fsetxattr -S removexattr -S lremovexattr -S fremovexattr -F auid>=500 -F auid!=4294967295 -k perm_mod"
    ...
    then
      ...
      RULE_SYSCALLS=$(echo ${RULE} | grep -o -P '(-S \w+ )+')
        ...
        then
          ...
          # syscall group. Example: '-S chown -S setxattr'
          #
          # Therefore:
          # * delete the original rule for arch & key from audit.rules
          #   (original '-S chown -S setxattr' rule would be deleted)
          # * delete $SYSCALL_GROUP -S arguments from the rule,
          #   but keep those not from this $SYSCALL_GROUP
          #   (original '-S chown -S setxattr' would become '-S chown')
          # * append the modified (filtered) rule again into audit.rules
          #   if the same rule not already present
          #   (new rule for same arch & key with '-S chown' would be appended
          #    if not present yet)
          sed -i -e "/${RULE}/d" ${AUDIT_RULES_FILE}
          # Drop ' -S (lsetxattr|fsetxattr|setxattr|fremovexattr|lremovexattr|removexattr)'
          # from the rule's system calls list
          NEW_SYSCALLS_FOR_RULE=$(echo ${RULE_SYSCALLS} | sed -r -e "s/[\s]*-S (lsetxattr|fsetxattr|setxattr|fremovexattr|lremovexattr|removexattr)//g")
