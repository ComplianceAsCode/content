...
#
# NOTE: The file 'template_umask' should be located in the same working directory as this script.
# The template contains tags that *must* be replaced successfully in order for the checks to work.
#
#       directory path, file name, external OVAL umask variable name, title, description, custom oval check id
#

import sys, csv, re

def output_check(path_info):
        # the csv file contains lines that match the following layout (see file_umask_checks.csv for more details):
        #
        #   directory path, file name, external OVAL umask variable name, title, description, custom oval check id
        #
        ...
        with open("./template_umask", 'r') as templatefile:
                ...
                with open("./output/" + oval_check_id + ".xml", 'wb+') as outputfile:
...
def main():
        if len(sys.argv) < 2:
                ...
                print "   directory path, file name, external OVAL umask variable name for comparison"
                sys.exit(1)
