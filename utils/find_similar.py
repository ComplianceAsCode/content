...
def merge_proper(ssg_root, product, path, equivalent_path):
    ...
    if 'prodtype' in right_sections:
        _prodtype_section = find_section_lines(right, "prodtype")[0]
    ...
    for common in ['identifiers', 'references']:
        if not common in left_sections or not common in right_sections:
            continue
    ...
    for common in left_sections.intersection(right_sections):
        if common in ['prodtype', 'identifiers', 'references']:
            continue
