...
class CISNISTHarvester:
    ...
    def parse_cis_pdf_file(self, pdf_file: Path) -> Dict[str, Set[str]]:
        ...
        if not shutil.which('pdftotext'):
            print(f"  Skipping PDF parsing (pdftotext not available)")
            return {}
        ...
        try:
            ...
            print(f"    Warning: PDF parsing timed out")
            return {}
    ...
    def harvest_from_product(
        ...
        # Find CIS MD file
        md_pattern = f"CIS_Red_Hat_Enterprise_Linux_{rhel_version}_Benchmark_*.md"
        md_files = list(self.nist_sync_dir.glob(md_pattern))
        ...
        print(f"  Skipping MD parsing (using PDF only for more reliable NIST reference extraction)")
