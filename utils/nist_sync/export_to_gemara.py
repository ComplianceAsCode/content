...
def _write_xccdf_profile(product: str, repo_root: Path, verbose: bool) -> None:
    ...
    No 'platform:' field is set: like every other product-scoped profile under
    products/{product}/profiles/, the platform is implicit from its location.
    Setting it explicitly also breaks for products (e.g. fedora) whose product
    ID isn't itself a valid CPE platform name.
    """
