...
Output:
  - controls/nist_800_53.yml              (unified control file with guards)
...
# Step 2: Generate CIS reference files in split-by-family format
log_section "Step 2: Generating NIST 800-53 CIS Reference Files (Split Format)"

# Delete existing reference files so we generate fresh (can't parse files with Jinja2 guards)
log_info "Removing old reference files (contain guards that can't be parsed)..."
...
log_info "Building CIS reference files in split-by-family format..."
if python3 sync_nist_split.py; then
    log_success "CIS reference files generated (split by family)"
    log_info "Metadata: shared/references/controls/nist_800_53_cis_reference.yml"
    log_info "Families: shared/references/controls/nist_800_53_cis_reference/*.yml"
else
    log_error "Failed to generate CIS reference files"
    exit 1
fi

# Step 3: Generate variable-to-products mapping
log_section "Step 3: Generating Variable-to-Products Mapping"
log_info "Scanning CIS control files to build variable→products map..."
if python3 generate_variable_to_products.py; then
    log_success "Variable mapping generated"
    log_info "Mapping file: data/variable_to_products.json"
else
    log_error "Failed to generate variable mapping"
    exit 1
fi

# Step 3.4: Add guards to CIS reference family files
log_section "Step 3.4: Adding Product Guards to CIS Reference Family Files"
log_info "Generating Jinja2 guards for family files..."
FAMILY_GUARDS_ADDED=0
for family_file in "$REPO_ROOT/shared/references/controls/nist_800_53_cis_reference"/*.yml; do
    if [ -f "$family_file" ]; then
        if python3 generate_product_family_guards.py --target rhel \
            --control-file "$family_file" \
            --output "$family_file" 2>/dev/null; then
            FAMILY_GUARDS_ADDED=$((FAMILY_GUARDS_ADDED + 1))
        fi
    fi
done
log_success "Product guards added to $FAMILY_GUARDS_ADDED family files"

# Step 3.5: No longer needed - profiles use nist_800_53:all directly
log_section "Step 3.5: Profiles Configuration"
log_info "CIS-NIST profiles use: nist_800_53:all"
log_info "This references the real control files in controls/nist_800_53/"
log_info "No temporary copy needed for building"

# Step 4: Compare CIS reference with previous version
log_section "Step 4: Comparing CIS Reference Changes"
if [ -f "$REPO_ROOT/shared/references/controls/nist_800_53_cis_reference.yml" ]; then
    if git diff --quiet HEAD -- "$REPO_ROOT/shared/references/controls/nist_800_53_cis_reference.yml" 2>/dev/null; then
        log_info "No changes in CIS reference file"
    else
        log_warning "CIS reference file has changes!"
        log_info "Review the diff to see what changed in CIS mappings:"
        log_info "  git diff shared/references/controls/nist_800_53_cis_reference.yml"
        log_info ""
        log_info "You may need to manually update controls/nist_800_53.yml"
    fi
else
    log_info "First time generating CIS reference file"
fi

# Step 4.5: Generate CIS-NIST profiles
log_section "Step 4.5: Generating CIS-NIST Profiles"
...
# Step 7: No cleanup needed - no temporary files copied
log_section "Step 7: File Status"
log_info "Reference files (for comparison only):"
log_info "  - shared/references/controls/nist_800_53_cis_reference.yml (metadata)"
log_info "  - shared/references/controls/nist_800_53_cis_reference/*.yml (families)"
log_info ""
log_info "Real files (used in builds, human-editable):"
log_info "  - controls/nist_800_53.yml (metadata)"
log_info "  - controls/nist_800_53/*.yml (families)"

# Step 8: Summary
log_section "Summary"
echo "Generated artifacts:"
echo ""
echo "  CIS Reference Files (for comparison, split by family, with guards):"
echo "    - shared/references/controls/nist_800_53_cis_reference.yml (metadata)"
echo "    - shared/references/controls/nist_800_53_cis_reference/*.yml (21 family files)"
echo ""
echo "  Real Control Files (used in builds, human-maintained):"
echo "    - controls/nist_800_53.yml (metadata)"
echo "    - controls/nist_800_53/*.yml (21 family files with CIS mappings + guards)"
echo ""
echo "  Mapping Cache:"
...
echo ""
log_info "Next steps:"
echo "  - Review CIS reference changes: git diff shared/references/controls/nist_800_53_cis_reference.yml"
echo "  - Manually update real file if needed: vim controls/nist_800_53.yml"
...
echo ""
echo "ℹ️  NOTE: The reference file (with guards) shows what CIS currently maps to NIST."
echo "   Profiles inherit from: nist_800_53_cis_reference:all"
echo "   Update the real control file (controls/nist_800_53.yml) manually"
echo "   based on the diff if you want to apply the changes."
echo ""
