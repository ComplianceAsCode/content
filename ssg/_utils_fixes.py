...
from ssg._yaml import open_and_macro_expand, open_raw
from ssg._checks import is_cce_valid


def has_empty_identifier(yaml_file, product_yaml=None):
    rule = open_and_macro_expand(yaml_file, product_yaml)
    ...
    if 'identifiers' in rule and rule['identifiers'] is not None:
        for i_type, i_value in rule['identifiers'].items():
            if i_type[0:3] == 'cce':
                ...
                remainder_valid = is_cce_valid("CCE-" + i_value[3:])
                remainder_valid |= is_cce_valid("CCE-" + i_value[4:])
...
def has_invalid_cce(yaml_file, product_yaml=None):
    ...
    if 'identifiers' in rule and rule['identifiers'] is not None:
        for i_type, i_value in rule['identifiers'].items():
            if i_type[0:3] == 'cce':
                if not is_cce_valid("CCE-" + i_value):
                    return True
    return False
...
def rewrite_value_remove_prefix(line):
    ...
    value = line[key_end+1:].strip()
    new_value = value
    if is_cce_valid("CCE-" + value[3:]):
        new_value = value[3:]
    elif is_cce_valid("CCE-" + value[4:]):
        new_value = value[4:]
...
def fix_file(path, product_yaml, func):
    ...
    yaml_contents = open_and_macro_expand(path, product_yaml)
