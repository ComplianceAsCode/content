...
class CPEALLogicalTest(Function):
    ...
    def get_bash_conditional(self, product_cpes, conditionals_path, env_yaml):
        ...
        contents += "( "
        child_condlines = []
        # FIXME include: child_includes = set()
        for a in self.args:
            ...
            if r is not None:
                ...
                # FIXME include: if 'include' in r.config:
                # FIXME include:    child_includes.update(r.config['include'].split(','))
        if self.is_or():
            op = " || "
        elif self.is_and():
            op = " && "
        ...
        contents += " )"
        # FIXME include: config['include'] = ",".join(child_includes)
