...
* Create a srpm with `fedpkg srpm`. This srpm will be used for rhel8 builds, rename it appropriately.
...
* Create another srpm with `fedpkg srpm` and name it appropriately. This srpm willb e used for rhel6 and rhel7 builds.
...
* Verify that you have build permissions through settings -> permissions.
     * If you don't have build permissions, you can ask for them for them directly. Wait for one of administrators to confirm.
* Make new builds. One for rhel6 and rhel7, one for rhel8 only. Use URLs pointing to SRPMs which you have just uploaded.
