...
This project uses a two-file versioning pattern for profiles (see `stig.profile` / `stig-v2r3.profile` for reference):
...
1. Parse the product list (comma-separated). Common products: `ocp4`, `rhcos4`, `rhel9`.
...
Check if a control file exists that matches the profile name (e.g., for profile `cis`, look for `controls/cis_ocp.yml` or `controls/cis_ocp/`). If found:

1. Read the control file's top-level YAML to check the `product` field.
2. If the `product` field does not list all the products from the argument, warn the user and offer to update it. A control file needs `product: [ocp4, rhcos4]` to work with both product profiles. See `controls/stig_ocp4.yml` for the multi-product pattern.
...
- **Product full names** for titles/descriptions:
  - `ocp4` → "Red Hat OpenShift Container Platform 4"
  - `rhcos4` → "Red Hat Enterprise Linux CoreOS 4"
