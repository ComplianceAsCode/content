        ...
        try:
            ...
            if len(lineinfileinfo) == 3:
                ...
                if missing_parameter_pass == "true":
                    ...
                    raise ValueError(
                        "Unrecognized value for missing_parameter_pass (%s). "
                        "Possible values are: true or false." % missing_parameter_pass)
            else:
                raise CSVLineError()
        except ValueError as e:
            sys.stderr.write("Error unpacking rule_id, parameter, "
                             "value and missing_parameter_pass: %s\n" % str(e))
            raise CSVLineError()
