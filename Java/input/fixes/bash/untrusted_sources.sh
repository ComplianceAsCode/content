...
for JAVA_DIR in $(echo ${JAVA_DIRs}); do
	if [ -e ${JAVA_DIR} ]; then
		if [ "$(grep -c "^deployment.security.askgrantdialog.notinca=false$" ${JAVA_DIR}/deployment.properties)" = "0" ]; then
			echo "deployment.security.askgrantdialog.notinca=false" >> ${JAVA_DIR}/deployment.properties
		fi
	fi
done