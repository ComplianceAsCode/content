...
echo "-w /etc/hostname -p wa -k audit_rules_networkconfig_modification" >> /etc/audit/rules.d/networkconfig.rules
...
if {{{ bash_package_installed("NetworkManager") }}} ; then
    echo "-w /etc/NetworkManager/NetworkManager.conf -p wa -k audit_rules_networkconfig_modification" >> /etc/audit/rules.d/networkconfig.rules
    echo "-w /etc/NetworkManager/conf.d/ -p wa -k audit_rules_networkconfig_modification" >> /etc/audit/rules.d/networkconfig.rules
    echo "-w /etc/NetworkManager/dnsmasq.d/ -p wa -k audit_rules_networkconfig_modification" >> /etc/audit/rules.d/networkconfig.rules
    echo "-w /etc/NetworkManager/dispatcher.d/ -p wa -k audit_rules_networkconfig_modification" >> /etc/audit/rules.d/networkconfig.rules
fi
