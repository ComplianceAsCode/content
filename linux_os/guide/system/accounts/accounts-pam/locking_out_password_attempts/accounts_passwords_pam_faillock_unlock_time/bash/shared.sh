...
if [ -f /etc/security/faillock.conf ]; then
    if $(grep -q '^\s*unlock_time\s*=' /etc/security/faillock.conf); then
        sed -i --follow-symlinks "s/^\s*\(unlock_time\s*\)=.*$/\1 = $var_accounts_passwords_pam_faillock_unlock_time/g" /etc/security/faillock.conf
    else
        echo "unlock_time = $var_accounts_passwords_pam_faillock_unlock_time" >> /etc/security/faillock.conf
    fi
