...
if grep -q "^SHA_CRYPT_MIN_ROUNDS" {{{ login_defs_path }}} ; then
    sed -i "s/SHA_CRYPT_MIN_ROUNDS.*/SHA_CRYPT_MIN_ROUNDS 1/g" {{{ login_defs_path }}}
else
    echo "SHA_CRYPT_MIN_ROUNDS 1" >> {{{ login_defs_path }}}
fi
echo "SHA_CRYPT_MIN_ROUNDS 4999" >> {{{ login_defs_drop_in_path }}}

if grep -q "^SHA_CRYPT_MAX_ROUNDS" {{{ login_defs_path }}} ; then
    sed -i "s/SHA_CRYPT_MAX_ROUNDS.*/SHA_CRYPT_MAX_ROUNDS 1/g" {{{ login_defs_path }}}
else
    echo "SHA_CRYPT_MAX_ROUNDS 1" >> {{{ login_defs_path }}}
fi
echo "SHA_CRYPT_MAX_ROUNDS 5000" >> {{{ login_defs_drop_in_path }}}
